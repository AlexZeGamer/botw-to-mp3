...
# GitHub gist link: https://gist.github.com/SamusAranX/6eb8b6fd1777b17afc3107a979c2409a
...
def extract_from_bars(file_name, output_dir):
	with open(file_name, "rb") as f:
		...
		for t in range(bars_count):
			...
			bar_name = os.path.splitext(os.path.basename(file_name))[0] # Get the name of the BARS file
			bfwav_name = track_name + track_ext # Construct output file name

			output_file_root = os.path.join(output_dir, bar_name)
			output_file = os.path.join(output_file_root, bfwav_name)
			os.makedirs(output_file_root, exist_ok=True) # Create the output directory if it doesn't exist
			with open(output_file, "wb") as wf:
				wf.write(fwav_data) # write the data to a BFWAV file
				print(f"{f.name}: Saved track {t+1} to {output_file}")
...
def main():
	...
	for _f in files:
		try:
			extract_from_bars(_f, os.path.dirname(_f))
